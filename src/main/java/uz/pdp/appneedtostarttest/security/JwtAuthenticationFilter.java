...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private void setUserPrincipalIfAllOk(HttpServletRequest request) {
        String authorization = request.getHeader(AppConstant.AUTHENTICATION_HEADER);
    ...
    public UserPrincipal getUserFromBearerToken(String token) {
        ...
        try {
            token = token.trim();
            if (token.startsWith(AppConstant.BEARER_TOKEN)) {
                token = token.substring(AppConstant.BEARER_TOKEN.length()).trim();
